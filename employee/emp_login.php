...
<?php
	...
	$employee="select * from employees where empID='$uname' and fname='$password'";

	

		
	
	 $que_user = mysqli_query($conn, $employee);
	 ...
	 if($count==1 || $count>1){
		...
		$empID=$_POST['user_name'];
		//$sessionname=$usename;
		$_SESSION['empID'] = $empID;
		$_SESSION['name']=$name;
		session_write_close();
		header("location: viewSquad.php");
		
	 }
	 else{
		echo '<script language=javascript>alert("WRONG USERNAME OR PASSWORD COMBINATIONS")</script>';
		echo '<script language=javascript>window.location="emp_login.php"</script>';
		}
...
<body>
<?php include 'empLoginHeader.php';?>
<div class="content">
